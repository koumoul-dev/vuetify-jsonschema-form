...
selectUtils.fillSelectItems = (fullSchema, value, selectItems, itemKey) => {
  if (!value) return
  if (fullSchema.type === 'array') {
    value.map(item => item).reverse().forEach(item => {
      if (!selectItems.find(selectItem => matchItem(selectItem, item, itemKey))) {
        selectItems.push(item)
      }
    })
  } else if (!selectItems.find(selectItem => matchItem(selectItem, value, itemKey))) {
