...
export const getRules = (fullSchema, options, required, isOneOfSelect) => {
  ...
  // ajv validation
  if (options.ajv && options.useAjv) {
    rules.push(getAjvRule(fullSchema, options.ajv, options.ajvLocalize, options.locale))
  }
...
const getAjvRule = (schema, ajv, localize, locale) => {
  const validate = ajv.compile(schema)
  return (val) => {
    const valid = validate(val)
    if (!valid && localize && locale && localize[locale]) {
