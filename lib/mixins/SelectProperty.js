...
export default {
  ...
  methods: {
    ...
    updateSelectItems() {
      const selectItems = selectUtils.getSelectItems(this.rawSelectItems, this.fullSchema, this.modelWrapper, this.modelKey, this.itemKey, this.itemIcon)
    ...
    renderSelectItem(h, item) {
      ...
      if (this.options.vuetifyVersion === 1) {
    ...
    renderSelectProp(h) {
      ...
      const scopedSlots = {
        selection: (data) => {
          ...
          if (this.fullSchema.type === 'array' && data.index !== this.modelWrapper[this.modelKey].length - 1) text += ',&nbsp;'
          return h('div', {
      ...
      if (this.fromUrlWithQuery) {
        tag = 'v-autocomplete'
        props.filter = () => true
        props.noDataText = this.options.noDataMessage
        props.placeholder = this.options.searchMessage
        props.searchInput = this.q
        on['update:search-input'] = (searchUpdate) => { this.q = searchUpdate }
      } else if (this.rawSelectItems && this.rawSelectItems.length > 20) {
        tag = 'v-autocomplete'
