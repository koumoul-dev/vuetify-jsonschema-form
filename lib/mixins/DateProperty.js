export default {
  ...
  methods: {
    renderDateProp(h) {
      ...
      let child
      const pickerProps = { locale: this.fullOptions.locale, value: this.value }
      ...
      if (this.fullSchema.format === 'time') {
        ...
        const dateParts = this.value ? this.value.split('T') : []
        const tabs = [
          h('v-tab', { props: { href: '#tab-date' } }, [h('v-icon', [this.fullOptions.icons.calendar])]),
          h('v-tab', { props: { href: '#tab-time', disabled: !dateParts[0] } }, [h('v-icon', [this.fullOptions.icons.clock])]),
      ...
      scopedSlots.activator = ({ on }) => {
        return h('v-text-field', {
          props: { ...this.commonFieldProps, clearable: !this.required, readonly: true, prependIcon },
