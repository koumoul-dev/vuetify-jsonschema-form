export default {
  ...
  methods: {
    ...
    renderSection(h, schema, childProp) {
      if (!childProp) return
      ...
      if (this.display === 'expansion-panels') {
        return [h('v-expansion-panel', [
          h('v-expansion-panel-header', { class: { 'error--text': this.validated && this.childrenErrors[schema.key] } }, [schema.title]),
          h('v-expansion-panel-content', { props: { eager: true } }, [childProp])
        ])]
      } else if (this.display === 'tabs') {
        return [
          h('v-tab', { props: { href: `#tab-${this.fullKey}-${schema.key}` } }, [
            h('span', { class: { 'error--text': this.validated && this.childrenErrors[schema.key] } }, [schema.title])
          ]),
          h('v-tab-item', { props: { value: `tab-${this.fullKey}-${schema.key}`, eager: true } }, [h('v-card', { props: { tile: true, flat: true } }, [h('v-card-text', [childProp])])])
        ]
      } else {
        ...
        on: {
          ...
          input: v => {
            if (v === null || v === undefined || v === '') {
              ...
              // of a tuple (except the last part), otherwise we mode last parts of the array to the beginning of the array
      ...
      if (this.fullSchema.properties) {
        this.fullSchema.properties.forEach(prop => {
          if (this.isSection(prop)) {
            sectionsChildren = sectionsChildren.concat(this.renderSection(h, prop, this.renderChildProp(h, prop, null, this.sectionDepth + 1)))
          } else {
      ...
      if (Array.isArray(this.fullSchema.items)) {
        this.fullSchema.items.forEach((item, i) => {
          ...
          if (this.isSection(item)) {
            sectionsChildren = sectionsChildren.concat(this.renderSection(h, schema, this.renderChildProp(h, schema, null, this.sectionDepth + 1, forceRequired)))
          } else {
      ...
      if (this.fullSchema.allOf) {
        this.fullSchema.allOf.forEach((allOf, i) => {
          ...
          if (this.isSection(allOf)) {
            ...
            sectionsChildren = sectionsChildren.concat(this.renderSection(h, schema, childProp))
          } else {
