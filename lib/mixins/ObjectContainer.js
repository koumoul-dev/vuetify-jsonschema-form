  ...
  methods: {
    ...
    renderObjectContainer(h) {
      ...
      if (this.fullSchema.properties) {
        this.fullSchema.properties.forEach((prop, i) => {
          if (this.isSection(prop)) {
            const childProp = this.renderChildProp(h, prop, null, this.sectionDepth + 1, false)
            sectionsChildren = sectionsChildren.concat(this.renderSection(h, prop, null, childProp))
          } else {
            flatChildren.push(this.renderChildProp(h, prop, null, this.sectionDepth, false))
          }
        })
      }
