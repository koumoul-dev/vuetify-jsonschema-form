  ...
  methods: {
    ...
    initObjectContainer(model) {
      ...
      if (this.subSchemas && !this.currentOneOf && this.subSchemasConstProp) {
        // currentOneOf is reactive based on the key property
        this.$watch(`value.${this.subSchemasConstProp.key}`, (val) => {
          // this.$set(this.subModels, 'currentOneOf', JSON.parse(JSON.stringify(this.value)))
          this.currentOneOf = this.subSchemas.find(item => item.properties[this.subSchemasConstProp.key].const === this.value[this.subSchemasConstProp.key])
        })
    ...
    renderObjectContainer(h) {
      ...
      if (this.subSchemas && this.subSchemas.length) {
        const props = {
          ...
          value: this.value[this.subSchemasConstProp.key],
        ...
        const on = {
          input: value => {
            this.$set(this.value, this.subSchemasConstProp.key, value)
          }
        }
