export default {
  ...
  computed: {
    ...
    subSchemasRules() {
      const rules = []
      if (this.subSchemasRequired) rules.push((val) => (val !== undefined && val !== null && val !== '') || this.options.requiredMessage)
      return rules
    }
  },
  watch: {
    ...
    renderObjectContainer(h) {
      ...
      if (this.subSchemas && this.subSchemas.length) {
        const props = {
          ...
          label: this.subSchemasConstProp ? (this.subSchemasConstProp.title || this.subSchemasConstProp.key) : 'Type',
