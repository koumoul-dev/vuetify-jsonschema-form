export default {
  ...
  methods: {
    ...
    renderSection(h, schema, childProp) {
      if (!childProp) return
      if (schema['x-display'] === 'hidden' || (schema.readOnly && this.fullOptions.hideReadOnly)) return
      ...
      if (Array.isArray(this.fullSchema.items)) {
        this.fullSchema.items.forEach((item, i) => {
          if (this.isSection(item)) {
            ...
            sectionsChildren = sectionsChildren.concat(this.renderSection(h, schema, this.renderChildProp(h, schema, null, this.sectionDepth + 1)))
          } else {
