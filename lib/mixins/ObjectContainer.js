  ...
  methods: {
    ...
    renderChildProp(h, prop, subModelKey, sectionDepth) {
      return h('property', {
        props: {
          schema: prop,
          value: subModelKey ? this.subModels[subModelKey] : this.value[prop.key],
        ...
        on: {
          ...
          input: v => {
            if (subModelKey) {
              this.$set(this.subModels, subModelKey, v)
            } else {
              if (v === null || v === undefined || v === '') this.$delete(this.value, prop.key)
              else this.$set(this.value, prop.key, v)
            }
