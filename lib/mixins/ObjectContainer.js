export default {
  ...
  methods: {
    ...
    renderStepperStep(h, schema, childProp, step, isLast) {
      if (!childProp) return
      const props = { step }
      ...
      if (this.display === 'expansion-panels') {
        ...
        return [
          h('v-stepper-content',
            ...
            [h('v-card', { props: { tile: true, flat: true } }, [h('v-card-text', [
              childProp,
              isLast ? null : h('v-btn', { props: { color: 'primary' }, on: { click: () => { this.currentStep += 1 } } }, 'confirm')
            ])])]
          )
        ]
      } else {
    ...
    renderObjectContainer(h) {
      ...
      sections.forEach((section, i) => {
        ...
        if (this.display === 'stepper') stepperSteps = stepperSteps.concat(this.renderStepperStep(h, section.schema, childProp, i + 1, i === sections.length - 1))
      })
