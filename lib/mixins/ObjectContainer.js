    ...
    renderSection(h, title, childKey, childProp) {
      if (this.fullSchema['x-display'] === 'expansion-panels') {
        return [h('v-expansion-panel', [
          h('v-expansion-panel-header', [title]),
          h('v-expansion-panel-content', [childProp])
        ])]
      } else if (this.fullSchema['x-display'] === 'tabs') {
        return [
          h('v-tab', { props: { href: `#tab-${this.fullKey}-${childKey}` } }, [title]),
          h('v-tab-item', { props: { value: `tab-${this.fullKey}-${childKey}` } }, [h('v-card', { props: { tile: true, flat: true } }, [h('v-card-text', [childProp])])])
        ]
      } else {
        return [h('v-layout', { attrs: { row: true, wrap: true }, class: 'ma-0' }, [
          h('span', {
          // style: this.foldable ? 'cursor:pointer;' : '',
            class: ['headline', 'title', 'subtitle-1', 'subtitle-2'][this.sectionDepth] || 'subtitle-2'
