...
export default {
  ...
  computed: {
    isObjectContainer() {
      ...
      if (this.fullSchema.type !== 'object' && !(this.fullSchema.prefixItems || Array.isArray(this.fullSchema.items))) return false
  ...
  methods: {
    isSection(prop, insideAllOf) {
      ...
      if (prop.properties || prop.prefixItems || Array.isArray(prop.items)) return true
      return false
    },
    ...
    renderSection(h, schema, subModelKey, childProp, step, isLast) {
      ...
      if (schema.visible === false || schema['x-display'] === 'hidden' || (schema.readOnly && this.fullOptions.hideReadOnly)) return [childProp]
        ...
        on: {
          ...
          input: v => {
            if (v === undefined) {
              ...
              if ((wrapper) && parseInt(modelKey) < (wrapper.length - 1)) {
    ...
    renderObjectContainer(h) {
      ...
      if (this.fullSchema.prefixItems || Array.isArray(this.fullSchema.items)) {
        (this.fullSchema.prefixItems || this.fullSchema.items).forEach((schema, i) => {
