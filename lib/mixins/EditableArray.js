export default {
  ...
  methods: {
    ...
    renderEditableArray(h) {
      ...
      const header = h('v-row', { class: 'mt-2 mb-1 pr-1' }, headerChildren)

      let list
      if (this.value && this.value.length) {
        const listItems = this.value.filter(item => !!item).map((item, i) => h('v-col', { props: this.fullOptions.arrayItemColProps }, [
