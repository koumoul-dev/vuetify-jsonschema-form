...
describe('render performance - Vue re-render counting', () => {
  ...
  it('should handle many-props schema efficiently (200 nested objects)', async () => {
    // Build the same schema as the /_dev/many-props page:
    // 200 object properties (obj_0..obj_199), each containing a str field, + 1 str1 field
    const properties = { str1: { type: 'string' } }
    const initialData = { str1: 'hello' }
    for (let i = 0; i < 200; i++) {
      properties[`obj_${i}`] = {
        type: 'object',
        properties: { str: { type: 'string' } }
      }
      initialData[`obj_${i}`] = { str: `val_${i}` }
    }
    ...
    // Settle validation with a first input
    const root = statefulLayout.stateTree.root
    const targetChild = root.children.find(c => c.key === 'obj_50')
    statefulLayout.input(targetChild.children[0], 'first_change')

    // Clear debug cache to measure a clean second input
    const debugCache = statefulLayout._lastCreateStateTreeContext._debugCache
    for (const key of Object.keys(debugCache)) delete debugCache[key]

    const root2 = statefulLayout.stateTree.root
    const targetChild2 = root2.children.find(c => c.key === 'obj_50')

    const t0 = performance.now()
    statefulLayout.input(targetChild2.children[0], 'second_change')
    const t1 = performance.now()

    const inputMs = t1 - t0

    // Count cache hits and misses
    const entries = Object.entries(debugCache)
    const hits = entries.filter(([, v]) => v[v.length - 1] === 'hit').length
    const misses = entries.filter(([, v]) => v[v.length - 1] === 'miss').length
    const skips = entries.filter(([, v]) => v[v.length - 1] === 'skip').length

    console.log('\nMany-props performance:')
    console.log(`  State nodes: ${entries.length}`)
    console.log(`  Cache hits: ${hits}, misses: ${misses}, skips: ${skips}`)
    console.log(`  statefulLayout.input() time: ${inputMs.toFixed(1)}ms`)

    // Verify structural sharing: unchanged obj_N children should be cache hits
    const root3 = statefulLayout.stateTree.root
    const unchangedChild = root3.children.find(c => c.key === 'obj_0')
    const changedChild = root3.children.find(c => c.key === 'obj_50')
    expect(root2.children.find(c => c.key === 'obj_0')).toBe(unchangedChild) // same reference
    expect(root2.children.find(c => c.key === 'obj_50')).not.toBe(changedChild) // new reference

    // Expect high cache hit rate: ~200 hits (199 unchanged obj_N + str1)
    // Only ~3 misses: root section + obj_50 section + obj_50/str field
    // Note: when a parent obj_N is a cache hit, its children aren't re-evaluated
    expect(hits).toBeGreaterThanOrEqual(195)
    expect(misses).toBeLessThanOrEqual(5)

    // Performance: input should complete well under 100ms even in test environment
    expect(inputMs).toBeLessThan(100)
  })
})
