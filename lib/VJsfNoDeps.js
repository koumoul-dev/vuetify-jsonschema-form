...
import Parser from './utils/rich-expression-parser'
...
export default {
  ...
  methods: {
    ...
    getFromExpr(exp) {
      ...
      } else if (this.initialOptions.evalMethod === 'evalExpr') {
        const parser = new Parser(require('expr-eval').Parser);
        return parser.evaluate(exp, expData)
      } else {
