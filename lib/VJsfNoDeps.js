...
export default {
  ...
  render(h) {
    ...
    const children = []
    if (this.$scopedSlots.beforehtml) children.push(this.$scopedSlots.before(this.slotParams))
    else if (this.$slots.beforehtml) this.$slots.before.forEach(node => children.push(node))
    else if (this.xSlots.beforehtml) children.push(h('div', { domProps: { innerHTML: this.xSlots.before } }))
  ...
  methods: {
    ...
    fixProperties() {
      ...
      if (
        this.fullSchema.type === 'object' && this.fullSchema.properties && this.fullSchema.properties.length &&
    ...
    initFromSchema() {
      ...
      if (this.fullSchema.type === 'object' && [undefined, null].includes(model)) {
        model = {}
      }
      // const always wins
