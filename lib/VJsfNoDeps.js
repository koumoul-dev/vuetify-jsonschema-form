  ...
  methods: {
    ...
    getFromExpr(exp) {
      ...
      if (this.initialOptions.evalMethod === 'newFunction') {
        ...
        //  this.evaluate(exp, this.value);
        return this.isRichExpr(exp) ? this.evaluate(exp, expData) : this._vjsf_getters[exp](expData)
      }
    },
    // used by getFromExpr to support simpler expressions that look into the root model by default
    prefixExpr(key) {
      if (key.startsWith('context.') || key.startsWith('model.') || key.startsWith('value.') || key.startsWith('modelRoot.') || key.startsWith('root.') || key.startsWith("'") || !isNaN(parseFloat(key))) return key
