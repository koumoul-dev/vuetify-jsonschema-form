...
export default {
  ...
  computed: {
    fullOptions() {
      ...
      fullOptions.memMarkdown = fullOptions.memMarkdown || memoize((text) => {
        if (!text) return ''
        return fullOptions.markdown(text)
      }, { maxAge: 60000 })
      ...
      var dflt = {}
      for (let [key, value] of Object.entries(this.$vuetify.icons.values)){
        dflt[key] = value.props ? value.props.name : value
      }
      fullOptions.icons = {  ...(iconSets[iconfont] || {}), ...dflt, ...fullOptions.icons}
    ...
    htmlDescription() {
      return this.fullOptions.memMarkdown(this.fullSchema && this.fullSchema.description)
    },
