...
export default {
  ...
  watch: {
    fullSchema: {
      handler() {
        // For oneOfs we do not know if the currentOneOf has changed, so we trigger
        // reinitialization of currentOneOf and subModels by setting lastFullSchema to null
        if (this.fullSchema && this.fullSchema.type === 'object' && this.fullSchema.oneOf) {
          this.lastFullSchema = null
        }
