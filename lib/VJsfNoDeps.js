...
const debugFlag = localStorage.debug
...
export default {
  ...
  computed: {
    fullOptions() {
      ...
      fullOptions.icons = { ...(iconSets[iconfont] || {}), ...vuetifyCustomIcons, ...fullOptions.icons }
    ...
    label() {
      return this.resolvedSchema.title || (typeof this.modelKey === 'string' ? this.modelKey : '')
    },
    display() {
      return this.modelKey === 'root' && this.fullOptions.rootDisplay ? this.fullOptions.rootDisplay : this.resolvedSchema['x-display']
    },
    customTag() {
      return this.resolvedSchema['x-tag']
    },
    rules() {
    ...
    disabled() {
      return this.fullOptions.disableAll || this.resolvedSchema.readOnly
    },
    slotName() {
      return this.resolvedSchema['x-display'] && this.resolvedSchema['x-display'].startsWith('custom-') ? this.resolvedSchema['x-display'] : this.fullKey
    },
    slotParams() {
    ...
    propertyClass() {
      return `vjsf-property vjsf-property-${this.dashKey} pa-0 ${this.resolvedSchema['x-class'] || ''} ${this.resolvedSchema.readOnly ? 'read-only' : ''}`
        .replace(/ {2}/g, ' ').trim()
    },
    xSlots() {
      return { ...this.resolvedSchema['x-slots'] }
    },
    formattedValue() {
      return this.value && this.resolvedSchema.format && this.fullOptions.formats[this.resolvedSchema.format] && this.fullOptions.formats[this.resolvedSchema.format](this.value, this.fullOptions.locale)
    }
  },
  watch: {
    fullSchema: {
  ...
  methods: {
    debug(msg) {
      if (debugFlag === '*' || debugFlag === this.fullKey) console.log(`[${this.fullKey}] ${msg}`)
    },
