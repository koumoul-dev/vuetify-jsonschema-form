...
// 1 => 01, 12 => 12
export const padTimeComponent = (val: number) => {
  const s = '' + val
...
export const getDateTimeWithOffset = (date: Date) => {
...
export const getDateTime = (parts: [string, string]) => {
  const date = new Date()
...
export const getShortTime = (time: string | undefined) => {
  if (!time) return ''
  return time.slice(0, 5)
}

export const getLongTime = (time: string) => {
  return time + ':00Z'
}
