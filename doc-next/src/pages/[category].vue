...
<template>
  <v-container v-if="category">
    ...
    <MarkdownInline :content="category.description" class="mb-6" />
    ...
    <template v-for="example in category.examples" :key="`${category.id}/${example.id}`">
      <a :id="example.id" class="anchor" />
      <h2 class="text-headline-large mt-8 mb-3">{{ example.title }}</h2>
      <v-alert v-if="example.warning" type="warning" variant="outlined" class="mb-4">
        {{ example.warning }}
      </v-alert>
      <MarkdownInline :content="example.description" class="mb-2" />
      <VjsfExample :example="example" :category-id="category.id" />
    </template>
  </v-container>
