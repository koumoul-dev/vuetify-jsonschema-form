...
`?await g(w.lineBreak):await g(w.addChar),p++}return p}else if(i.removed){for(let p=a+i.value.length;p>a;p--)e.value=e.value.slice(0,p-1)+e.value.slice(p),await g(w.removeChar);return-i.value.length}else return i.value.length},x=async()=>{for(const i of ce.map(a=>a.trim())){console.log("apply step",i);const a=ue(e.value,i);let p=0;for(const h of a){const m=await d(h,p);console.log("index",m),p+=m}e.value=i}};return O(e,()=>{try{t.value=se.parse(e.value)}catch{}}),x(),K({title:"VJSF - Player"}),Q(()=>{document.getElementsByTagName("html")[0].style.overflowY="hidden"}),X(()=>{document.getElementsByTagName("html")[0].style.overflowY=""}),(i,a)=>t.value?(R(),P(N(le),{key:0,fluid:"",class:"pa-0"},{default:z(()=>[D(N(oe),{dense:""},{default:z(()=>[D(N(J),{class:"ma-0"},{default:z(()=>[Y("div",{style:k(`max-height: ${u.value}px;overflow-y: auto;`)},[D(N(ie),{modelValue:e.value,"onUpdate:modelValue":a[0]||(a[0]=p=>e.value=p),class:"vjsf-code-editor py-2",style:{"min-height":"200px"},highlight:n,"line-numbers":"",readonly:!0},null,8,["modelValue"])],4)]),_:1}),D(N(J),null,{default:z(()=>[Y("div",{style:k(`max-height: ${u.value}px;overflow-y: auto;`),class:"pt-2 pr-2"},[v.value?(R(),P(N(ee),{key:0,ref_key:"form",ref:y,modelValue:c.value,"onUpdate:modelValue":a[2]||(a[2]=p=>c.value=p)},{default:z(()=>[D(N(te),b({modelValue:s.value,"onUpdate:modelValue":a[1]||(a[1]=p=>s.value=p)},v.value),null,16,["modelValue"])]),_:1},8,["modelValue"])):A("",!0)],4)]),_:1})]),_:1})]),_:1})):A("",!0)}};export{Be as default};
