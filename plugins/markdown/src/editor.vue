...
export default defineComponent({
  ...
  setup (props, { expose, emit }) {
    ...
    const messages = computed(() => {
      return { ...i18n.en, ...i18n[props.locale], ...props.messages }
    })
    ...
    const fieldSlots = computed(() => {
      ...
      fieldSlots.default = () => {
        const children = [
          h(VLabel, { text: props.label })
        ]
        if (props.readOnly) {
          children.push(h('div', { innerHTML: marked.parse(props.modelValue) }))
        } else {
    ...
    const initEasyMDE = async () => {
      ...
      /** @type {import('./types.js').VjsfMarkdownIcons} */
      const icons = {
        ...props.icons,
      ...
      const config = {
        ...
        minHeight: '300px',
        insertTexts: {
          link: [messages.value.mdeLink1, messages.value.mdeLink2],
          image: [messages.value.mdeImg1, messages.value.mdeImg2],
          table: [messages.value.mdeTable1, messages.value.mdeTable2],
          horizontalRule: ['', '\n\n-----\n\n']
        },
        toolbar: [{
          name: 'bold',
          ...
          title: messages.value.bold
        }, {
          name: 'italic',
          action: EasyMDE.toggleItalic,
          icon: iconSvg(icons.formatItalic),
          title: messages.value.italic
        }, {
          name: 'heading',
          action: EasyMDE.toggleHeadingSmaller,
          icon: iconSvg(icons.formatTitle),
          title: messages.value.heading
        }, /* {
    name: 'heading-1',
    ...
    title: 'Titre 3'
        ...
        '|',
        {
          ...
          title: messages.value.quote
        },
        {
          name: 'unordered-list',
          action: EasyMDE.toggleUnorderedList,
          icon: iconSvg(icons.formatListBulleted),
          title: messages.value.unorderedList
        },
        {
          name: 'ordered-list',
          action: EasyMDE.toggleOrderedList,
          icon: iconSvg(icons.formatListNumbered),
          title: messages.value.orderedList
        },
        '|',
        {
          name: 'link',
          ...
          title: messages.value.createLink
        },
        {
          name: 'image',
          action: EasyMDE.drawImage,
          icon: iconSvg(icons.image),
          title: messages.value.insertImage
        },
        {
          name: 'table',
          action: EasyMDE.drawTable,
          icon: iconSvg(icons.table),
          title: messages.value.createTable
        },
        '|',
        {
          name: 'preview',
